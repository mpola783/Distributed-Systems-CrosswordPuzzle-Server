...
public class CrosswordGameStateImpl extends UnicastRemoteObject implements CrosswordGameState {
	...
	*/
    ...
    @Override
    public void removeLetterGuess(char letter) {
        int lastIndex = -1;
        ...
        if (lastIndex != -1) {
            // Shift elements left to remove the last instance of the letter
            for (int j = lastIndex; j < lettersGuessedCount - 1; j++) {
                lettersGuessed[j] = lettersGuessed[j + 1];
            }
            lettersGuessedCount--;
            lettersGuessed[lettersGuessedCount] = '\0'; // Optional: clear last element
        } else {
            throw new IllegalArgumentException("Letter not found in guessed list!");
        }
    }

    @Override
    public void removeWordGuess(String word) {
        int lastIndex = -1;
        for (int i = 0; i < wordsGuessedCount; i++) {
            if (wordsGuessed[i].equals(word)) {
                lastIndex = i;
            }
        }
    
        if (lastIndex != -1) {
            // Shift elements left to remove the last instance of the word
            for (int j = lastIndex; j < wordsGuessedCount - 1; j++) {
                wordsGuessed[j] = wordsGuessed[j + 1];
            }
            wordsGuessedCount--;
            wordsGuessed[wordsGuessedCount] = null; // Optional: clear last element
        } else {
            throw new IllegalArgumentException("Word not found in guessed list!");
        }
    }
