...
public class CrissCrossPuzzleServerImpl extends UnicastRemoteObject implements CrissCrossPuzzleServer {
	...
	// Instead of storing game-specific data internally, we keep a map of game
	// states.
	...
	private static final int TIMELIMIT_SECONDS = 2;
...
///////////////////////////NEW LOGIC TEST////////////////////////////////////////////////////////////////////////
	...
	@Override
	public String startMultiplayer(String name, int numberOfPlayers, int gameLevel, long sequenceNumber)
	...
	@Override
	public void updatePlayerScore(String gameID, String playerName, int points, long sequenceNumber)
			throws RemoteException {
		CrosswordGameState gameState = gameStates.get(gameID);
	...
	public String joinMultiplayer(String name, String gameID) throws RemoteException {
		/*
		 * CrosswordGameState gameState = gameStates.get(gameID); if (gameState == null)
		 * { throw new RemoteException("Game with ID " + gameID + " not found."); } if
		 * (gameState.getPlayers().size() >= gameState.getExpectedPlayers()) { throw new
		 * RemoteException("Game is already full."); } gameState.addPlayer(name);
		 * 
		 * // Update lobby info. GameLobbyInfo lobby = pendingLobbies.get(gameID); if
		 * (lobby != null) { lobby.incrementPlayers(); }
		 * 
		 * // If the lobby is now full, remove it from pending and start the game. if
		 * (gameState.getPlayers().size() == gameState.getExpectedPlayers()) {
		 * pendingLobbies.remove(gameID); // Generate the puzzle for the game.
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// NEWGAME START
	/*
	 * Main function to create a new game Connects to word server and generates user
	 * game grid Creates randomly selected crossword layout using game input from
	 * user
	 * 
	...
	@Override
	public String startGame(String player, int numberOfWords, int failedAttemptFactor, String gameID,
		...
		if (!validateWordCount(numberOfWords)) {
			...
			throw new RemoteException("Words chosen are not within 2 - 10.");
		}
	...
	/*
	 * Main function to check and update grid based on users guess Updates global
	 * variables userGrid/finishedGrid and lettersGuessed/wordsGuessed to return a
	 * new game grid Called in Main
	 * 
	 * Returns a WIN if grid is completed with new guess Returns existing grid
	 * layout and life counter if not
	 */
	@Override
	public String checkGuess(CrosswordGameState gameState, String guess, String name, long sequenceNumber)
			throws RemoteException {
	...
	public char[][] createGrid(String vert_word, GridDimensions grid_x, String[] horiz_words, int[] vert_cross_index,
			int[] horiz_cross_index) {
	...
	private int[] determineCrossovers(String vert_word, int[] vert_cross_index, String[] horiz_words) {
		if (horiz_words.length < vert_cross_index.length) {
			throw new IllegalArgumentException(
					"Mismatch between vertical cross index count and horizontal words count.");
		}
	...
	// CREATE FUNCTIONS
	// Prepares grid for user by masking all characters except those in
	// letters_guessed or word_guessed
	 ...
	 * HashSet<String> set = new HashSet<>(); ArrayList<String> matchingWords = new
	 * ArrayList<>();
	 * 
	 * // Add all elements from the second array to a HashSet for quick lookup for
	 * (String word : array2) { if (word != null) { // Avoid null values
	 * set.add(word); } }
	 * 
	 * // Check if words in the first array exist in the set for (String word :
	 * array1) { if (word != null && set.contains(word)) { matchingWords.add(word);
	 * } }
	 * 
	 * // Convert ArrayList to String[] return matchingWords.toArray(new String[0]);
	 * }
	 */
	...
	private void revealWords(CrosswordGameState gameState, char[][] grid, char[][] maskedGrid) throws RemoteException {
		...
		for (String word : gameState.getWordsGuessed()) {
			if (word == null || word.isEmpty())
				continue; // Skip empty/null words
	...
	// Function to check if the grid is fully revealed (i.e., contains no '-'
	// characters)
	...
	public boolean heartbeat(String name, String gameID) throws RemoteException {
		CrosswordGameState gameState = gameStates.get(gameID);
		if (gameState == null) {
			System.out.println("Lobby " + gameID + " closed.");
			return false;
		}
